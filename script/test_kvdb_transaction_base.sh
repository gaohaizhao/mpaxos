...
DIR_RESULT=result.kvdb.transaction.base
is_exit=0
is_async=1
n_group=1
sz_data=100
...
do
    ...
    do
        ...
        command="screen -m -d /bin/bash -c \"~/test_mpaxos/test_mpaxos.out ~/test_mpaxos/config.$N_HOST.$i 1 $n_tosend $n_group $is_async $is_exit 5 $group_begin $n_batch $sz_data  >& ~/test_mpaxos/result.mpaxos.$n_batch.$i\"" 
        echo $command
